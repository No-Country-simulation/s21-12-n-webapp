...
public class NotificacionController {
    ...
    @Operation(summary = "Crear notificación", description = "Genera una notificación para un usuario",
    ...
    @Operation(summary = "Obtener notificaciones por usuario", description = "Recupera las notificaciones asociadas a un usuario",
            security = @SecurityRequirement(name = "Bearer Token"),
            responses = {
                    @ApiResponse(responseCode = "200", description = "Notificaciones obtenidas exitosamente"),
                    @ApiResponse(responseCode = "404", description = "Usuario o notificaciones no encontrados")
            })
    @GetMapping("/usuario/{usuarioId}")
    public ResponseEntity<List<NotificacionResponse>> getNotificacionesByUsuario(@PathVariable Long usuarioId) {
        List<NotificacionResponse> responses = notificacionService.getNotificacionesByUsuario(usuarioId);
